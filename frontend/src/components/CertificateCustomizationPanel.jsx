...
const CertificateCustomizationPanel = ({
  ...
  const [config, setConfig] = useState({
    ...
    body:
      safeConfig.body ||
      "for successfully participating in our workshop and demonstrating dedication to learning and growth.",
...
const CertificatePreview = React.forwardRef(({ config }, ref) => {
  ...
  return (
    ...
    >
      ...
      {/* Name Placeholder - Will be added dynamically by backend */}
      <div className="text-center">
        <div style={{ marginBottom: "40px", minHeight: "112px" }}>
          {/* Empty space for dynamic participant name */}
        </div>
      ...
      {/* Load Google Fonts */}
      <link
        href={`https://fonts.googleapis.com/css2?family=${getFontFamily(
          config.nameFont
        ).replace(/ /g, "+")}:wght@700&family=${getFontFamily(
          config.titleFont
        ).replace(/ /g, "+")}:wght@700&family=${getFontFamily(
          config.sigFont
        ).replace(
          / /g,
          "+"
        )}:wght@500&family=Merriweather:wght@400&family=Inter:wght@400&display=swap`}
        rel="stylesheet"
      />
    </div>
  );
